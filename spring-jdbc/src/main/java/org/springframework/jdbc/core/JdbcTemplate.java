 ...
 * It simplifies(简化) the use of JDBC and helps to avoid common errors.
 ...
 *
 * <p>Can be used within a service implementation via(通过) direct(直接) instantiation(实例化)
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	/**
	 * 默认值为true: SQL 警告信息还是警告信息，不会抛出异常
	 * 如果设置为false，一旦出现了SQL警告信息，将会直接抛出异常.
	 */
	...
	public JdbcTemplate(DataSource dataSource) {
		/**
		 * 设置数据源，调用的是JdbcAccessor的setDataSource方法，数据源是通过Spring容器注入的，可以根据需要配置带有连接池的数据源.
		 */
		setDataSource(dataSource);
		/**
		 * 执行JdbcAccessor的afterPropertiesSet方法， JdbcAccessor是实现了InitializingBean接口的.
		 *
		 * 更重要的目的是为了校验数据源是否设置或者是否注入.
		 */
		afterPropertiesSet();
	}
	 ...
	 * <p>Default is -1, indicating(表示) to use the JDBC driver's default configuration
	 ...
	 *
	 * 当提前知道没有结果时，可以用来跳过结果处理过程，
	 * 在Oracle的一些老版本的驱动中，使用该值可以避免一些bug.
	 */
	...
	public <T> T execute(ConnectionCallback<T> action) throws DataAccessException {
		...
		// 获取连接，obtainDataSource调用的是父类JdbcAccessor中的方法，
		//  在创建JdbcTemplate时将DataSource传入JdbcAccessor中的
	...
	public <T> T execute(StatementCallback<T> action) throws DataAccessException {
		// 判断回调函数是否为空.
		Assert.notNull(action, "Callback object must not be null");
		// 通过注入的数据源（数据库连接等信息）去获取数据库连接
		Connection con = DataSourceUtils.getConnection(obtainDataSource());
		Statement stmt = null;
		try {
			// 创建JDBC Statement，普通的Statement，不是PreparedStatement.
			stmt = con.createStatement();
			// 设置Statement中的一些参数，如：fetchSize，maxRows，或者queryTimeOut
			applyStatementSettings(stmt);
			// 将Statement对象传给回调函数，在回调函数中对方法的结果集就行包装返回
			T result = action.doInStatement(stmt);
			// 处理SQL查询过程中的警告信息，如果设置了ignoreWarnings为true，只会打印警告信息
			// 如果对于警报信息比较敏感，可以设置ignoreWarnings的值为false，表示如果有警告信息，将会抛出SQLWarningException异常.
			handleWarnings(stmt);
			// 返回包装之后的查询结果
			return result;
		}
	...
	public <T> T query(final String sql, final ResultSetExtractor<T> rse) throws DataAccessException {
		 ...
		 *
		 * SqlProvider接口: 提供了获取SQL语句的方法
		 *
		 * StatementCallBack接口: 提供了doInStatement方法.
		 *
		 * 该类中只提供对SQL结果集的操作，不进行数据库连接的获取和关闭.
		 */
		class QueryStatementCallback implements StatementCallback<T>, SqlProvider {
			...
			public T doInStatement(Statement stmt) throws SQLException {
				...
				try {
					...
					// 使用ResultSetExtractor对SQL的结果集进行封装.
		...
		// 在execute方法中回调QueryStatementCallBack中的doInStatement()方法和getSql()方法.
	...
	/**
	 * rowMapper类型为：ColumnMapRowMapper
	 * @throws DataAccessException
	 */
	@Override
		...
		class BatchUpdateStatementCallback implements StatementCallback<int[]>, SqlProvider {
			...
			public int[] doInStatement(Statement stmt) throws SQLException, DataAccessException {
				...
				if (JdbcUtils.supportsBatchUpdates(stmt.getConnection())) {
					...
					// 如果当前数据库驱动不支持批量更新操作。则将传入的SQL语句进行循环逐行执行
	...
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		try {
			...
			// 设置PreparedStatement需要的参数，如：fetchSize，maxRows，queryTimeOut
			applyStatementSettings(ps);
			// 执行SQL语句，并返回封装之后的结果
			T result = action.doInPreparedStatement(ps);
			// 处理SQL警告信息，如果设置了ignoreWarnings为true，则直接忽略警报
			// 如果设置了ingoreWarnings为false，则出现了warning时，会抛出SQLWarningException异常
			handleWarnings(ps);
			return result;
		}
		catch (SQLException ex) {
	...
	public <T> T execute(CallableStatementCreator csc, CallableStatementCallback<T> action)
		...
		try {
			...
			// 设置CallAbleStatement参数，fetchSize，maxRows，queryTime
			applyStatementSettings(cs);
			// 使用回调函数执行存储过程或者函数，并包装结果返回
