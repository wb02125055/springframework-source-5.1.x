		...
		if (!isCache()) {
			...
			if (view == null) {
				synchronized (this.viewCreationCache) {
					// 首先从视图对象的缓存中根据缓存key获取对应的视图对象
					view = this.viewCreationCache.get(cacheKey);
					if (view == null) {
						// 调用子类UrlBasedViewResolver的createView对象，然后根据视图名称来创建对应类型的视图对象
						// 比如：以"redirect:"开头的，或者以"forward:"开头的，会创建不同类型的视图对象
						...
						// 如果视图创建成功
						if (view != null) {
							// 视图创建完成之后，将创建的视图对象放入到缓存中.
							// 缓存中的cacheKey: viewName + "_" + Local对象
